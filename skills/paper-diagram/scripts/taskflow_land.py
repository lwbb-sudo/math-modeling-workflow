...
"""taskflow-land：横版任务流水线图（若干任务块，块内横向/纵向流水线，块间粗箭头串联）。
    ...
    python3 taskflow_land.py content.json -o out.drawio            # 默认灰白论文版
    python3 taskflow_land.py content.json --theme color -o out-color.drawio
...
# 彩色主题采样自参考图：深蓝紫步骤盒 + 中紫小盒 + 浅蓝紫面板 + 深蓝强调
...
FRAME = '#111111'
TITLE_C = '#5a2d6e'
STEP_S, STEP2_S, MID_S, BLACK_S = STEP_A, STEP2_A, MID_A, BLACK_A
ACCENT_TEXT, BRACE = WHITE, '#2a2a52'
cells, problems = [], []


def configure_theme(name):
    """Apply a palette while preserving task layout and emphasis semantics."""
    global INK, WHITE, STEP_A, STEP_B, STEP_S, STEP2_A, STEP2_B, STEP2_S
    global MID_A, MID_B, MID_S, LITE, LITE_S, LITE2, LITE2_S
    global PANEL, PANEL_S, PANEL2, PANEL2_S, BLACK_A, BLACK_S
    global ARROW, FRAME, TITLE_C, ACCENT_TEXT, BRACE

    if name == 'color':
        INK, WHITE = '#262626', '#ffffff'
        STEP_A, STEP_B, STEP_S = '#2c2c56', '#6f6f9e', '#2c2c56'
        STEP2_A, STEP2_B, STEP2_S = '#2a5a84', '#5f93bd', '#2a5a84'
        MID_A, MID_B, MID_S = '#5a549c', '#9a92c4', '#5a549c'
        LITE, LITE_S = '#cdd0e8', '#8a86b4'
        LITE2, LITE2_S = '#c6d2ee', '#7f93c4'
        PANEL, PANEL_S = '#e6e9f8', '#9a96c0'
        PANEL2, PANEL2_S = '#dde5f6', '#7f93c4'
        BLACK_A, BLACK_S = '#0b0b12', '#0b0b12'
        ARROW, FRAME, TITLE_C = '#8b8bc0', '#111111', '#5a2d6e'
        ACCENT_TEXT, BRACE = WHITE, '#2a2a52'
    elif name == 'mono':
        INK, WHITE = '#222222', '#ffffff'
        STEP_A = STEP_B = '#d9d9d9'
        STEP2_A = STEP2_B = MID_A = MID_B = '#f2f2f2'
        STEP_S = STEP2_S = MID_S = '#333333'
        LITE, LITE_S = '#ffffff', '#333333'
        LITE2, LITE2_S = '#ffffff', '#333333'
        PANEL, PANEL_S = '#ffffff', '#555555'
        PANEL2, PANEL2_S = '#ffffff', '#555555'
        BLACK_A, BLACK_S = '#d9d9d9', '#333333'
        ARROW, FRAME, TITLE_C = '#222222', '#555555', '#222222'
        ACCENT_TEXT, BRACE = INK, '#333333'
    else:
...
def box(cid, x, y, w, h, text, fill, stroke, fs=FS, fc=None, bold=1, align='center', grad=None):
    """直角盒；grad 给出渐变终点色时按水平渐变绘制（参考图的主要质感来源）。"""
    fc = INK if fc is None else fc
    ls = lines_of(text)
    fit(cid, ls, w, h, fs)
...
def brace(cid, x, y, w, h, color=None):
    """左侧大括号：参考图用它把步骤盒和右侧子框连起来，而不是箭头。"""
    color = BRACE if color is None else color
    raw(cid, x, y, w, h,
        f'shape=curlyBracket;whiteSpace=wrap;html=1;rounded=1;direction=north;'
        f'fillColor=none;strokeColor={color};strokeWidth=1.6;')


def label(cid, x, y, w, h, text, fs=FS_T, fc=None, align='left'):
    fc = TITLE_C if fc is None else fc
    ls = lines_of(text)
...
def barrow(cid, x, y, w, h, direction='east', fill=None):
    fill = ARROW if fill is None else fill
    raw(cid, x, y, w, h,
        f'shape=singleArrow;direction={direction};arrowWidth=0.6;arrowSize=0.42;html=1;'
        f'fillColor={fill};strokeColor=none;')


def edge(cid, pts, stroke=None, width=1.2, end='block'):
    ...
    for j, r in enumerate(rs):
        ...
        if 'cols' in r:                       # 并排深色小盒，白字
            for i, (cx, cw) in enumerate(slots(ix, ix + iw, len(r['cols']), 8)):
                box(f'{cid}_r{j}c{i}', cx, iy, cw, hh, r['cols'][i], MID_A, MID_S,
                    FS_S, fc=ACCENT_TEXT, grad=MID_B)
...
def step_style(st):
    if st.get('emph') == 'black':
        return BLACK_A, BLACK_S, ACCENT_TEXT, None
    if st.get('emph'):
        return STEP2_A, STEP2_S, ACCENT_TEXT, STEP2_B
    return STEP_A, STEP_S, ACCENT_TEXT, STEP_B
...
def draw_sec(k, s, x0, x1, y):
    ...
    if t == 'pairs':
        ...
        for i, it in enumerate(L):
            box(f'{k}_pl{i}', lx, y + i * 46, lw, 36, it, MID_A, MID_S, FS,
                fc=ACCENT_TEXT, grad=MID_B)
        ry = y
        ...
        for bi, b in enumerate(row):
            ...
            if b.get('cylinder') and bi + 1 < len(row):   # 两个半宽块之间的圆柱中转标签
                ...
                raw(f'r{ri}b{bi}_cyl', cx2, cy2, cw2, 118,
                    ...
                    f'fillColor={PANEL};gradientColor={WHITE};gradientDirection=east;'
