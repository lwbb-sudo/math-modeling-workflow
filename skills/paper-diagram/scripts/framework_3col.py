...
"""framework-3col：三栏式研究框架图（左阶段链 / 中研究内容 / 右研究方法）。
    ...
    python3 framework_3col.py content.json -o out.drawio            # 默认灰白论文版
    python3 framework_3col.py content.json --theme color -o out-color.drawio
...
LINE = '#1a1a1a'
STAGE_S = 'none'
COLUMN_ARROW, CHAIN_ARROW = '#f79a8a', '#3cb878'
PAIR_BG, PAIR_ARROW = '#eef6ff', '#9a8cf0'
...
def configure_theme(name):
    """Apply a palette while preserving all measured geometry and semantics."""
    global INK, PINK, PINK_S, GTITLE, GTITLE_S, GSUB, GSUB_S
    global PURPLE, PURPLE_S, YELLOW, YELLOW_S, PEACH, PEACH_S, BLUE, BLUE_S
    global MAGENTA, MAGENTA_S, GREEN, GREEN_S, SALMON, SALMON_S
    global TEAL, TEAL_2, SKY, SKY_2, METHOD, METHOD_S, GREY, GREY_2, LINE
    global STAGE_S, COLUMN_ARROW, CHAIN_ARROW, PAIR_BG, PAIR_ARROW, FILLS
    ...
    if name == 'color':
        ...
        INK = '#222222'
        PINK, PINK_S = '#d9d9d9', '#333333'
        GTITLE, GTITLE_S = '#f2f2f2', '#333333'
        GSUB, GSUB_S = '#f2f2f2', '#333333'
        PURPLE, PURPLE_S = '#f2f2f2', '#333333'
        YELLOW, YELLOW_S = '#ffffff', '#333333'
        PEACH, PEACH_S = '#ffffff', '#333333'
        BLUE, BLUE_S = '#ffffff', '#333333'
        MAGENTA, MAGENTA_S = '#f2f2f2', '#333333'
        GREEN, GREEN_S = '#ffffff', '#333333'
        SALMON, SALMON_S = '#f2f2f2', '#333333'
        TEAL = TEAL_2 = SKY = SKY_2 = '#ffffff'
        METHOD, METHOD_S = '#ffffff', '#333333'
        GREY = GREY_2 = '#222222'
        LINE = '#333333'
        STAGE_S = '#333333'
        COLUMN_ARROW = CHAIN_ARROW = PAIR_ARROW = '#222222'
        PAIR_BG = '#ffffff'
    else:
        raise ValueError(f'未知主题: {name}')

    FILLS = {
        'yellow': (YELLOW, YELLOW_S), 'peach': (PEACH, PEACH_S),
        'blue': (BLUE, BLUE_S), 'green': (GREEN, GREEN_S),
        'magenta': (MAGENTA, MAGENTA_S), 'purple': (PURPLE, PURPLE_S),
        'salmon': (SALMON, SALMON_S),
    }
...
def edge(cid, pts, stroke=None, width=1.6, end='block', extra=''):
...
def dashframe(cid, x, y, w, h, color=None, pattern='6 5'):
...
def draw_section(k, s, x0, x1, y):
    ...
    if t == 'columns':
        ...
        for i, (cx, cw) in enumerate(slots(x0 + 12, x1 - 12, len(cols), 60)):
            ...
            if i:
                px = cx - 10 - 40
                block_arrow(f's{k}_a{i}', px, y + 16, 34, 24, 'east', COLUMN_ARROW)
        return h
    ...
    if t == 'chain':
        ...
        for i, (cx, cw) in enumerate(slots(x0 + 20, x1 - 20, len(items), 66)):
            ...
            if i:
                edge(f's{k}_ce{i}', [(cx - 62, y + 30), (cx - 4, y + 30)], CHAIN_ARROW, 4)
        return h
    ...
    if t == 'pair':
        ...
        rx = x0 + 16 + lw + 58
        rw = x1 - 16 - rx
        raw(f's{k}_lbg', lx - 8, y, lw + 16, h,
            f'rounded=0;html=1;fillColor={PAIR_BG};strokeColor=none;')
        raw(f's{k}_rbg', rx - 8, y, rw + 16, h,
            f'rounded=0;html=1;fillColor={PAIR_BG};strokeColor=none;')
        ...
        edge(f's{k}_pe', [(lx + lw + 12, y + h / 2), (rx - 14, y + h / 2)], PAIR_ARROW, 4)
        return h
...
def build(c):
    ...
    for i, st in enumerate(stages):
        ...
        raw(f'st{i}', RAIL_L[0], sy, MID[0] - RAIL_L[0] - 2, 82,
            ...
            f'fillColor={fill};gradientColor={grad};gradientDirection=east;strokeColor={STAGE_S};'
            f'strokeWidth=1;'
