#!/usr/bin/env python3
"""stageflow-3col：分阶段流程框架图（左方法链 / 中流程块 / 右阶段说明）。

    python3 stageflow_3col.py content.json -o out.drawio            # 默认灰白论文版
    python3 stageflow_3col.py content.json --theme color -o out-color.drawio
    python3 stageflow_3col.py content.json --check

与 framework-3col 的区别：中栏每块有独立标题条和流程型内部结构，左右栏是**竖排文字**
...
TITLE_FC, FRAME_S, FLOW_ARROW = '#ffffff', '#5a5a5a', None
...
def configure_theme(name):
    """Apply a palette while preserving stage layout and arrow semantics."""
    global INK, PINK, PINK_S, RAIL_S, RAIL_ARROW, RRAIL, RRAIL_S
    global TITLE_FC, FRAME_S, FLOW_ARROW, TONES
    ...
    if name == 'color':
        ...
        RAIL_S, RAIL_ARROW = '#333333', '#222222'
        RRAIL, RRAIL_S = '#ffffff', '#333333'
        TITLE_FC, FRAME_S, FLOW_ARROW = '#222222', '#666666', '#222222'
        paper_tone = ('#d9d9d9', '#333333', '#f2f2f2', '#ffffff', '#333333')
        TONES = {tone: paper_tone for tone in ('teal', 'green', 'olive', 'orange', 'blue')}
    else:
        raise ValueError(f'未知主题: {name}')


def flow_arrow_color(tone_bar):
    return tone_bar if FLOW_ARROW is None else FLOW_ARROW
...
def box(cid, x, y, w, h, text, fill, stroke, fs=FS, fc=None, arc=12, bold=1, extra=''):
...
def vbox(cid, x, y, w, h, text, fill, stroke, fs=FS_T, fc=None, arc=12):
    fc = INK if fc is None else fc
    chars = list(str(text))
    if len(chars) * (fs + 3) > h:
...
def edge(cid, pts, stroke=None, width=1.6, end='block', start='none', extra=''):
...
def dashframe(cid, x, y, w, h, color=None, pattern='5 4'):
    color = FRAME_S if color is None else color
    if w > 200 and h > 100:
...
def draw_sec(k, s, x0, x1, y, tone):
        ...
        for i, (gx, gw) in enumerate(slots(x0 + 14, x1 - 14, len(gs), 34)):
            ...
            if i:
                edge(f'{k}_ga{i}', [(gx - 30, y + h / 2 - 10), (gx - 10, y + h / 2 - 10)],
                     flow_arrow_color(TB), 2.4, 'block', 'block')
        return h
    ...
    if t == 'list_grid':
        ...
        edge(f'{k}_e', [(lx + lw + 6, y + h / 2), (rx - 14, y + h / 2)],
             flow_arrow_color(TB), 3)
        return h
...
def build(c):
    ...
    for bi, b in enumerate(blocks):
        ...
        box(f'blk{bi}_t', MID[0] + (MID[1] - MID[0] - tw_) / 2, y + 14, tw_, 34,
            b['title'], TB, TBS, FS_T, fc=TITLE_FC, arc=8)
        yy = y + 14 + 34 + 14
