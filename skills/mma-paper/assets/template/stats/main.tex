...
\usepackage{fontspec}
\usepackage{xeCJK}
\setCJKfamilyfont{fsgb}{STFangsong}
\newcommand{\fsgb}{\CJKfamily{fsgb}}
\setCJKfamilyfont{fzxbsong}{Songti SC}
...
% 写作提示：摘要最后完成，交代研究对象与数据、核心统计方法、主要估计值/效应及不确定性、结论与建议；
% 不只罗列方法名，不放复杂公式和表格。字数与中英文要求以当届赛事规则为准。
[中文摘要内容：500-700字]
...
% Writing guide: summarize the data, research objective, method, main estimates/effects with uncertainty, and conclusions.
% Do not merely list techniques or include complex formulas and tables; follow the current contest's length requirements.
...
% 最后用 bib --doi <DOI> --key <引用key> 生成条目并追加到 references.bib。
