...
CLASS_NAMES = ["Not significant", "High-High", "Low-Low", "Low-High", "High-Low"]
CLASS_COLORS = ["#FFFDB4", "#D50000", "#0759A8", "#79E8D5", "#F3B7B7"]
...
def local_cluster_classes(values: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """Quadrant classes from standardized values and their rook-neighbor spatial lag."""
    z = np.zeros_like(values)
    z[mask] = (values[mask] - values[mask].mean()) / values[mask].std()
    kernel = np.array([[0, 1, 0], [1, 0, 1], [0, 1, 0]], dtype=float)
    counts = convolve(mask.astype(float), kernel, mode="constant", cval=0)
    lag = convolve(z * mask, kernel, mode="constant", cval=0) / np.maximum(counts, 1)
    product = z * lag
    high_high = mask & (z > 0.62) & (lag > 0.46) & (product > 0.42)
    low_low = mask & (z < -0.82) & (lag < -0.60) & (product > 0.56)
    cross_sign = mask & (product < -0.12) & (np.abs(z) > 0.78) & (np.abs(lag) > 0.14)
    significant = high_high | low_low | cross_sign

    classes = np.zeros(values.shape, dtype=float)
    classes[significant & (z > 0) & (lag > 0)] = 1
    classes[significant & (z < 0) & (lag < 0)] = 2
    classes[significant & (z < 0) & (lag > 0)] = 3
    classes[significant & (z > 0) & (lag < 0)] = 4
    classes[~mask] = np.nan
    return classes


def simulate_lisa(outline: list[np.ndarray], bounds: tuple[float, float, float, float]) -> tuple[np.ndarray, np.ndarray, list[np.ndarray]]:
    """Deterministic LISA-style reconstruction; classes are not paper measurements."""
    ...
    rng = np.random.default_rng(164084009)

    high = (
        1.60 * gaussian(xn, yn, 0.15, 0.52, 0.075, 0.08)
        + 1.48 * gaussian(xn, yn, 0.39, 0.58, 0.072, 0.11)
        + 1.30 * gaussian(xn, yn, 0.58, 0.68, 0.060, 0.08)
        + 1.05 * gaussian(xn, yn, 0.66, 0.46, 0.065, 0.09)
        + 0.95 * gaussian(xn, yn, 0.78, 0.42, 0.065, 0.09)
        + 0.72 * gaussian(xn, yn, 0.29, 0.24, 0.055, 0.09)
    )
    low = (
        1.28 * gaussian(xn, yn, 0.31, 0.43, 0.060, 0.085)
        + 1.15 * gaussian(xn, yn, 0.55, 0.36, 0.055, 0.095)
        + 0.92 * gaussian(xn, yn, 0.78, 0.58, 0.050, 0.070)
        + 0.68 * gaussian(xn, yn, 0.09, 0.67, 0.060, 0.070)
    )
    texture = gaussian_filter(rng.normal(size=xx.shape), 2.0)
    texture = texture / texture.std()
    base = high - 0.76 * low + 0.16 * texture

    results = []
    for index, _year in enumerate(YEARS):
        drift = 0.055 * index * gaussian(xn, yn, 0.40, 0.58, 0.14, 0.15)
        drift -= 0.035 * index * gaussian(xn, yn, 0.55, 0.36, 0.12, 0.12)
        jitter = gaussian_filter(rng.normal(size=xx.shape), 1.3) * 0.025
        jitter += rng.normal(size=xx.shape) * 0.28
        results.append(local_cluster_classes(base + drift + jitter, mask))
    return x_edges, y_edges, results
...
def draw_boundaries(ax: plt.Axes, outline: list[np.ndarray],
    ...
    for _name, rings in counties:
        for ring in rings:
            ax.plot(ring[:, 0], ring[:, 1], color="#69767A", lw=0.32, zorder=4)
    for ring in outline:
...
def draw_cell_grid(ax: plt.Axes, x_edges: np.ndarray, y_edges: np.ndarray,
    ...
    ax.add_collection(LineCollection(segments, colors="#7D9699", linewidths=0.20,
                                     alpha=0.72, zorder=3))
...
def make_figure(output_stem: Path) -> None:
    ...
    x_edges, y_edges, fields = simulate_lisa(outline, bounds)
    xmin, ymin, xmax, ymax = bounds
...
def main() -> None:
    make_figure(ROOT / "outputs" / "esv_local_moran_scenarios_replica")
